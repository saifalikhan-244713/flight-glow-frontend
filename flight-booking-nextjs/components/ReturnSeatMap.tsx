...
function ReturnSeatMap() {
  ...
  const { seatMapResult, selectedSeats, returnSeatMapResult } = useSelector(
  ...
  const fetchSeatMap = async () => {
    try {
      // Call API
      const response = await SearchController.getSeatMaps(
        resultRow.resultRowReturn,
      );
  ...
  useEffect(() => {
    fetchSeatMap().then((response) => {
      dispatch(setReturnSeatMapResults(response.data));
    });
  }, []);

  const seats: Seat[] =
    returnSeatMapResult && returnSeatMapResult[0]?.decks[0]?.seats
      ? returnSeatMapResult[0].decks[0].seats
      : [];
  // const [selectedSeats, setSelectedSeats] = useState<any[]>([]);

  const returnSeats = useSelector(
    (state: RootState) => state.seatmap.returnSeats,
  );

  const handleCheckboxChange = (seat: Seat) => {
    const isSelected = returnSeats.includes(seat.number);
    const updatedSeats: string[] = isSelected
      ? returnSeats.filter((s: string) => s !== seat.number)
      : [...returnSeats, seat.number];

    if (!isSelected && returnSeats.length >= totalSeatsAllowed) {
      toast.warn(`You can't select more than ${totalSeatsAllowed} seats.`);
      return; // Prevent selecting more seats
    }

    if (returnSeats) {
      console.log('hello', returnSeats);
    }

    dispatch(setReturnSeats(updatedSeats));
  ...
  const generateSeatMap = () => {
    if (!seats.length) return null; // Handle case where seats data is not yet available

    // Extract unique rows and columns
    const rows: Record<string, Record<string, Seat>> = {}; // Type annotation for rows object
    seats.forEach((seat) => {
      // Ensure seat.number exists and is not null/undefined before using match
                          ...
                          >
                            <div
                              ...
                              className={`relative z-[1] h-6 w-6 cursor-pointer rounded-10xs text-lg text-white ${
                                returnSeats.includes(seat.number)
                                  ? 'bg-green-500'
                                  : 'bg-gray-100'
                              }`}
                            >
                              ...
                              <input
                                ...
                                checked={returnSeats.includes(seat.number)}
  ...
  return (
    <>
      <div className="custom-scrollbar box-border flex max-w-[869px] flex-row items-start justify-start self-stretch px-1 py-0">
