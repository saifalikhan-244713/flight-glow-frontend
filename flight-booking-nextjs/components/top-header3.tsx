...
export type TopHeader3Type = {
  /** Style props */
  frameDivHeight?: CSSProperties["height"];
  frameDivDisplay?: CSSProperties["display"];
};

const TopHeader3: NextPage<TopHeader3Type> = ({
  frameDivHeight,
  frameDivDisplay,
}) => {
  const flightGlowStyle: CSSProperties = useMemo(() => {
    return {
      height: frameDivHeight,
      display: frameDivDisplay,
    };
  }, [frameDivHeight, frameDivDisplay]);

  return (
    <header className="self-stretch bg-blue-2 overflow-hidden flex flex-row items-start justify-center pt-[18px] px-5 pb-[17px] box-border gap-[573px] top-[0] z-[99] sticky max-w-full text-left text-17xl text-white font-kanit mq1225:gap-[573px_286px] mq750:gap-[573px_143px] mq450:gap-[573px_72px]">
