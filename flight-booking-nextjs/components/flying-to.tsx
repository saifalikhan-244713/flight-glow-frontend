...
export type FlyingToType = {
  flightLandSvgrepoCom?: string;
  flyingTo?: string;
...
const FlyingTo: NextPage<FlyingToType> = ({
  flightLandSvgrepoCom,
  flyingTo,
  propOverflow,
}) => {
  const flightLandSvgrepoComIconStyle: CSSProperties = useMemo(() => {
    return {
  ...
  return (
    <div className="flex-[0.9373] rounded-3xs box-border overflow-hidden flex flex-col items-start justify-start py-[3px] px-[7px] min-w-[166px] text-center text-xs text-grey-grey-700 font-components-chip border-[1px] border-solid border-grey-7 mq750:flex-1">
      <div className="self-stretch h-[54px] overflow-hidden shrink-0 flex flex-row items-center justify-start py-[15px] px-0 box-border gap-[8px]">
        ...
        <img
          className="h-6 w-6 relative overflow-hidden shrink-0 min-h-[24px]"
          alt=""
          src={flightLandSvgrepoCom}
          style={flightLandSvgrepoComIconStyle}
        />
