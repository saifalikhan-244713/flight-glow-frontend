...
const IconSet: NextPage<IconSetType> = ({ propDisplay }) => {
  const flightGlow1Style: CSSProperties = useMemo(() => {
    return {
      display: propDisplay,
    };
  }, [propDisplay]);
  ...
  return (
    <footer className="self-stretch flex flex-col items-start justify-start max-w-full text-left text-17xl text-white font-kanit">
      <div className="self-stretch bg-blue-2 flex flex-col items-center justify-start py-[60px] px-5 box-border gap-[43.5px] max-w-full mq750:gap-[22px_43.5px]">
        <div className="w-[1240px] h-[63px] relative max-w-full mq1225:h-auto mq1225:min-h-[63]">
          ...
          <div className="absolute top-[9px] left-[0px] w-full flex flex-row items-start justify-between max-w-full gap-[20px] mq1225:flex-wrap">
            <h1
              className="m-0 relative text-inherit font-semibold font-inherit mq750:text-10xl mq450:text-3xl"
              style={flightGlow1Style}
            >
              FlightGlow
            </h1>
      ...
      <div className="self-stretch bg-orange overflow-hidden flex flex-row items-start justify-center py-[26px] pr-5 pl-[21px] text-base">
